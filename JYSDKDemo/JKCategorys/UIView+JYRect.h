//
//  UIView+JYRect.h
//  JYSDKDemo
//
//  Created by isenu on 2018/4/26.
//  Copyright © 2018年 isenu. All rights reserved.
//

#import <UIKit/UIKit.h>

@interface UIView (JYRect)

@property (nonatomic, assign) CGSize  jy_size;
@property (nonatomic, assign) CGFloat jy_width;
@property (nonatomic, assign) CGFloat jy_height;
@property (nonatomic, assign) CGFloat jy_centerX;
@property (nonatomic, assign) CGFloat jy_centerY;
@property (nonatomic, assign) CGFloat jy_top;
@property (nonatomic, assign) CGFloat jy_right;
@property (nonatomic, assign) CGFloat jy_left;
@property (nonatomic, assign) CGFloat jy_bottom;

@end
